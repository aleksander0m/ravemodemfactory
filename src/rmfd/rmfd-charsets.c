 ...
 *
 *
 * Charset support based on Modermfdanager:
...
static CharsetEntry charset_map[] = {
    { "UTF-8",   "UTF8",   "UTF-8",     "UTF-8//TRANSLIT",     RMFD_MODEM_CHARSET_UTF8 },
    { "UCS2",    NULL,     "UCS-2BE",   "UCS-2BE//TRANSLIT",   RMFD_MODEM_CHARSET_UCS2 },
    { "IRA",     "ASCII",  "ASCII",     "ASCII//TRANSLIT",     RMFD_MODEM_CHARSET_IRA },
    { "GSM",     NULL,     NULL,        NULL,                  RMFD_MODEM_CHARSET_GSM },
    { "8859-1",  NULL,     "ISO8859-1", "ISO8859-1//TRANSLIT", RMFD_MODEM_CHARSET_8859_1 },
    { "PCCP437", "CP437",  "CP437",     "CP437//TRANSLIT",     RMFD_MODEM_CHARSET_PCCP437 },
    { "PCDN",    "CP850",  "CP850",     "CP850//TRANSLIT",     RMFD_MODEM_CHARSET_PCDN },
    { "HEX",     NULL,     NULL,        NULL,                  RMFD_MODEM_CHARSET_HEX },
    { NULL,      NULL,     NULL,        NULL,                  RMFD_MODEM_CHARSET_UNKNOWN }
};

const char *
rmfd_modem_charset_to_string (RmfdModemCharset charset)
{
    CharsetEntry *iter = &charset_map[0];

    g_return_val_if_fail (charset != RMFD_MODEM_CHARSET_UNKNOWN, NULL);
...
RmfdModemCharset
rmfd_modem_charset_from_string (const char *string)
{
    CharsetEntry *iter = &charset_map[0];

    g_return_val_if_fail (string != NULL, RMFD_MODEM_CHARSET_UNKNOWN);
    ...
    return RMFD_MODEM_CHARSET_UNKNOWN;
}

static const char *
charset_iconv_to (RmfdModemCharset charset)
{
    CharsetEntry *iter = &charset_map[0];
...
static const char *
charset_iconv_from (RmfdModemCharset charset)
{
    CharsetEntry *iter = &charset_map[0];
...
gboolean
rmfd_modem_charset_byte_array_append (GByteArray *array,
                                    const char *utf8,
                                    gboolean quoted,
                                    RmfdModemCharset charset)
{
...
char *
rmfd_modem_charset_hex_to_utf8 (const char *src, RmfdModemCharset charset)
{
    ...
    unconverted = rmfd_utils_hexstr2bin (src, &unconverted_len);
    if (!unconverted)
        return NULL;

    if (charset == RMFD_MODEM_CHARSET_UTF8 || charset == RMFD_MODEM_CHARSET_IRA)
        return unconverted;
...
char *
rmfd_modem_charset_utf8_to_hex (const char *src, RmfdModemCharset charset)
{
    ...
    hex = rmfd_utils_bin2hexstr ((guint8 *)converted, converted_len);
    g_free (converted);
    return hex;
}
...
guint8 *
rmfd_charset_gsm_unpacked_to_utf8 (const guint8 *gsm, guint32 len)
{
    int i;
    GByteArray *utf8;
...
guint8 *
rmfd_charset_utf8_to_unpacked_gsm (const char *utf8, guint32 *out_len)
{
    GByteArray *gsm;
...
SubsetEntry subset_table[] = {
    { RMFD_MODEM_CHARSET_GSM,     gsm_is_subset },
    { RMFD_MODEM_CHARSET_IRA,     ira_is_subset },
    { RMFD_MODEM_CHARSET_UCS2,    ucs2_is_subset },
    { RMFD_MODEM_CHARSET_8859_1,  iso88591_is_subset },
    { RMFD_MODEM_CHARSET_PCCP437, pccp437_is_subset },
    { RMFD_MODEM_CHARSET_PCDN,    pcdn_is_subset },
    { RMFD_MODEM_CHARSET_UNKNOWN, NULL },
};

/**
 * rmfd_charset_get_encoded_len:
 *
 * @utf8: UTF-8 valid string
 * @charset: the #RmfdModemCharset to check the length of @utf8 in
 ...
 **/
guint
rmfd_charset_get_encoded_len (const char *utf8,
                            RmfdModemCharset charset,
                            guint *out_unsupported)
{
    ...
    g_return_val_if_fail (charset != RMFD_MODEM_CHARSET_UNKNOWN, 0);
    g_return_val_if_fail (utf8 != NULL, 0);

    if (charset == RMFD_MODEM_CHARSET_UTF8)
        return strlen (utf8);

    /* Find the charset in our subset table */
    for (e = &subset_table[0];
         e->cs != charset && e->cs != RMFD_MODEM_CHARSET_UNKNOWN;
         e++);
    g_return_val_if_fail (e->cs != RMFD_MODEM_CHARSET_UNKNOWN, 0);
 ...
 * that case. */
gchar *
rmfd_charset_take_and_convert_to_utf8 (gchar *str, RmfdModemCharset charset)
{
    gchar *utf8 = NULL;
    ...
    switch (charset) {
    case RMFD_MODEM_CHARSET_UNKNOWN:
        g_warn_if_reached ();
        utf8 = str;
        break;

    case RMFD_MODEM_CHARSET_HEX:
        /* We'll assume that the HEX string is really valid ASCII at the end */
        utf8 = str;
        break;

    case RMFD_MODEM_CHARSET_GSM:
    case RMFD_MODEM_CHARSET_8859_1:
    case RMFD_MODEM_CHARSET_PCCP437:
    case RMFD_MODEM_CHARSET_PCDN: {
    ...
    case RMFD_MODEM_CHARSET_UCS2: {
        ...
        if (possibly_hex) {
            utf8 = rmfd_modem_charset_hex_to_utf8 (str, charset);
            if (utf8) {
                g_free (str);
                break;
            }
     ...
     * already here */
    case RMFD_MODEM_CHARSET_IRA:
    case RMFD_MODEM_CHARSET_UTF8:
        utf8 = str;
        break;
    }
 ...
 * case. */
gchar *
rmfd_utf8_take_and_convert_to_charset (gchar *str,
                                     RmfdModemCharset charset)
{
    gchar *encoded = NULL;
    ...
    case RMFD_MODEM_CHARSET_UCS2: {
        ...
        /* Get hex representation of the string */
        hex = rmfd_utils_bin2hexstr ((guint8 *)encoded, encoded_len);
        g_free (encoded);
        encoded = hex;
        g_free (str);
        break;
...
gchar *
rmfd_utils_hexstr2bin (const gchar *hex, gsize *out_len)
{
...
gchar *
rmfd_utils_bin2hexstr (const guint8 *bin, gsize len)
{
    GString *ret;
    gsize i;
